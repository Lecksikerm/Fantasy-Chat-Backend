...
const initSocket = (server) => {
    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("toggle-reaction", async ({ messageId, emoji }) => {
            ...
            await message.save();
            io.to(message.conversationId.toString()).emit("message-reactions-updated", {
                messageId,
        ...
        socket.on("message-delivered", async ({ messageId }) => {
            ...
            if (message.status === "sent") {
                ...
                await message.save();
                socket.to(message.conversationId.toString()).emit("message-status", { messageId, status: "delivered" });
            }
        });
